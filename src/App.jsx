...
function applyColors(restaurant) {
  ...
  if (restaurant.color_ink) root.style.setProperty('--black', restaurant.color_ink)
  if (restaurant.color_gold) root.style.setProperty('--orange', restaurant.color_gold)
  if (restaurant.color_off) root.style.setProperty('--cream', restaurant.color_off)
}
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0A0A0A' }}>
      <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 24, fontWeight: 800, color: '#FF5C00', letterSpacing: '-0.5px' }}>Loading<span style={{ color: '#fff' }}>.</span></div>
    </div>
  )

  if (error) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0A0A0A' }}>
      <p style={{ color: '#333', fontFamily: 'DM Sans' }}>Restaurant not found.</p>
    </div>
  )
