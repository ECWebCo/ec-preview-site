export default function Footer({ restaurant }) {
  ...
  return (
    <footer style={{ background:'#141412',borderTop:'3px solid #C0392B',padding:'56px 64px 40px' }}>
      <div style={{ maxWidth:1100,margin:'0 auto' }}>
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:44,paddingBottom:44,borderBottom:'1px solid rgba(255,255,255,0.08)',flexWrap:'wrap',gap:32 }}>
          ...
          <nav style={{ display:'flex',gap:32,flexWrap:'wrap',alignItems:'flex-start' }}>
            {[['Menu','menu-section'],['Gallery','gallery-section'],['Location','location-section'],['Contact','contact-section']].map(([l,id])=>(
              <button key={id} onClick={()=>scrollTo(id)} style={{ background:'none',border:'none',fontSize:10,letterSpacing:'2.5px',textTransform:'uppercase',color:'rgba(255,255,255,0.3)',cursor:'pointer',fontFamily:'DM Sans',fontWeight:700,transition:'color 0.2s' }} onMouseOver={e=>e.target.style.color='#C0392B'} onMouseOut={e=>e.target.style.color='rgba(255,255,255,0.3)'}>{l}</button>
            ))}
          </nav>
        </div>
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12 }}>
          ...
          <div style={{ fontSize:12,color:'rgba(255,255,255,0.15)',fontFamily:'DM Sans' }}>Website by <a href="https://ecwebco.com" target="_blank" rel="noreferrer" style={{ color:'#C0392B',fontWeight:700 }}>EC Web Co</a></div>
        </div>
      </div>
