...
/* ─── Nav ───────────────────────────────────────────────────── */
function Nav({ restaurant, locations, onMenuOpen, onPick }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="site-nav-links" style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
          {navItems.map((item, i) => (
            <button
              key={i}
        ...
        >
          {[0, 1, 2].map(i => (
            <span
              key={i}
              style={{
                display: 'block', width: 24, height: 2, background: NAVY,
                transition: '0.3s',
                transform:
                  i === 0 && menuOpen
      ...
      {menuOpen && (
        ...
        >
          {navItems.map((item, i) => (
            <button
              key={i}
              onClick={() => (item.action ? (setMenuOpen(false), item.action()) : scrollTo(item.id))}
              style={{
                background: 'none', border: 'none',
            ...
            >
              <span
                style={{
                  ...
                  letterSpacing: '4px', textTransform: 'uppercase', color: NAVY,
                }}
              >
                {item.label}
              </span>
...
/* ─── Hero (slideshow + light logo overlay) ─────────────────── */
  ...
  return (
    <div
      ...
      style={{
        ...
        background: CREAM, paddingTop: 72,
      }}
    >
      {photos.map((p, i) => (
        <img
      ...
      {photos.length === 0 && (
        <div
          style={{
            ...
            background: 'linear-gradient(135deg,#1B2B4B,#0d1a2e)',
      ...
      >
        ...
        >
          {restaurant.city && (
            <div
              style={{
                fontFamily: 'DM Sans', fontSize: 10, letterSpacing: '4px',
          ...
          <h1
            style={{
              fontFamily: 'DM Sans', fontSize: 'clamp(15px,2vw,20px)', fontWeight: 700,
              letterSpacing: '7px', textTransform: 'uppercase', color: NAVY, marginBottom: 24,
            }}
          >
            {restaurant.name}
          </h1>
          {restaurant.about && (
            <p
              style={{
                fontFamily: 'Georgia,serif', fontSize: 15, color: NAVY,
                lineHeight: 1.9, marginBottom: 36, opacity: 0.85, maxWidth: 380,
              }}
            >
              {restaurant.about}
            </p>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            {hasSpecials && (
              <button
                onClick={onSpecials}
                style={{ ...PILL_BTN, borderColor: GOLD, color: GOLD }}
    ...
    <div style={{ marginBottom: 24 }}>
      <button
        ...
        style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          gap: 10, padding: '10px 0', width: '100%',
        }}
      >
        ...
        <span
          style={{
            ...
            letterSpacing: '2px', textTransform: 'uppercase', color: MUTED, marginLeft: 8,
          }}
        >
          Hours {open ? '↑' : '↓'}
        </span>
        ...
        display.map((h, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0' }}>
              ...
              <span
                style={{
                  fontFamily: 'DM Sans', fontSize: 13, fontStyle: 'italic',
                  color: isToday ? NAVY : (h.closed ? STONE : MUTED),
                }}
              >
                ...
                <div key={loc.id || i}>
                  {locations.length > 1 && (
                    <div
                      style={{
                        ...
                        color: MUTED, marginBottom: 8, opacity: 0.6,
                      }}
                    >
                      {loc.name}
                    </div>
                  ...
                  {loc.address && (
                    <p
                      style={{
                        ...
                        lineHeight: 1.8, marginBottom: 2, fontStyle: 'italic',
                      }}
                    >
                      {loc.address}
                    </p>
                  ...
                  {phone && (
                    <a
                      ...
                      style={{
                        fontFamily: 'Georgia,serif', fontSize: 14, color: MUTED,
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
                    ...
                    {links.order_url && (
                      <button
                        onClick={() => onPick('order')}
                        style={{ ...PILL_BTN, color: MUTED, borderColor: MUTED }}
                        onMouseOver={e => { e.currentTarget.style.background = MUTED; e.currentTarget.style.color = '#fff' }}
                        onMouseOut={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = MUTED }}
                      >
                        Order
                      </button>
                    )}
                    <button
                      onClick={() => onMenuOpen(loc)}
                      style={{ ...PILL_BTN, color: MUTED, borderColor: MUTED }}
                      onMouseOver={e => { e.currentTarget.style.background = MUTED; e.currentTarget.style.color = '#fff' }}
                      onMouseOut={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = MUTED }}
                    >
        ...
        {section.external_url && (
          <a
            ...
            style={{
              ...
              textDecoration: 'none', borderBottom: `1px solid ${NAVY}`,
              paddingBottom: 3, width: 'fit-content',
            }}
          >
            View Full Menu →
          </a>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      >
        ...
        {filteredSections.length === 0 ? (
          <div style={{ padding: 48, textAlign: 'center', color: MUTED, fontFamily: 'Georgia,serif', fontStyle: 'italic' }}>
            Menu coming soon.
          </div>
        ) : !isMobile ? (
...
function EventsModal({ restaurant, onClose }) {
  ...
  const eyebrow = hasEvents ? 'Entertain' : 'Contact'
  const heading = hasEvents ? 'Private Events & Catering' : 'Get in Touch'
  const subline = hasEvents
